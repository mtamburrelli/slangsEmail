...
app.config['MAIL_USERNAME'] = '7c53fdc400c951'
app.config['MAIL_PASSWORD'] = '579dca0931c636'
app.config['MAIL_USE_TLS'] = True
app.config['MAIL_USE_SSL'] = False


@celery.task(name="pdf-send")
def celery_email(mail_info):
    diccionario = showAllPalabras()
    with open('words.txt', 'w') as f:
        for palabra in diccionario:
            word = palabra["name"], palabra["definicion"]
            f.write(str(word))
            f.write("\n")
    ...
    contenido = Message(mail_info['subject'], sender='7c53fdc400c951', recipients=[mail_info['to']])
    contenido.attach('Slangs-Panameños.pdf', 'application/pdf', pdf)
    contenido.body = mail_info['body']
    email.send(contenido)
...
def sendCelery():
    ...
    mail_info = {
        'subject': '123 Probandooooo',
        'to': email,
        'body': "Prueba",
    }

    celery_email.delay(mail_info)
